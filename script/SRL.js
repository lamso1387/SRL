       ...
       SRL.MapActivityArea=function (mapId) {
        ...
        SRL.RequestNewAd=function () {
            if (FormValidation('#newAdForm input,#newAdForm select,#newAdForm button')) {
                ...
                FileUploader_ashx(true, null, formData, 'SaveNewAdd', null, null, function (result) { alert(result); });
                location.reload();
            }
        };
        ...
        SRL.RequestParentChilds=function (parentId) {
            ...
            ServerRequest(false, null, JSON.stringify(data), true, 'GetParentChilds', null, null, function (result) {
                if (result.length > 0) {
                    for (var i in result) {
                        childs.push({ text: result[i].childName, tags: [result[i].childId], nodes: RequestParentChilds(result[i].childId) });
                    }
                }
                else childs = null;
            });
        ...
        SRL.RequestCategoryList=function (async) {
            ...
            ServerRequest(async, null, null, true, 'GetCategoryParents', null, null, function (result) {
                for (var i in result) {
                    ...
                    if (parentId > 0)
                        parents.push({ text: result[i].parentName, tags: [result[i].parentId], nodes: RequestParentChilds(result[i].parentId) });
        ...
        SRL.RequestEditCatName=function () {
            if (FormValidation('#tbCategoryMgmtNodeName')) {
                ...
                ServerRequest(true, null, JSON.stringify(data), true, 'EditCatName', null, null, function () { RenderCategoryMgmt(); });
            }
        };
        ...
        SRL.RequestDeleteNode=function () {
            ...
            if (nodeId > 0) {
                var data = new Object();
                data['nodeId'] = nodeId;
                ServerRequest(true, null, JSON.stringify(data), true, 'DeleteCategory', null, null, function () { RenderCategoryMgmt(); });
            }
        };
        ...
        SRL.ScrollSmoth = function (linkSelector, speed) {// ('a[href*="#"]:not([href="#"])', 1000)
